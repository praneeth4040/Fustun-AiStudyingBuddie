...
var SUPABASE_URL = "https://dkwafhgmhijsbdqpazzs.supabase.co";
var SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImRrd2FmaGdtaGlqc2JkcXBhenpzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTM4OTM2OTksImV4cCI6MjA2OTQ2OTY5OX0.Pk0HgZhTgg2V_OsDyTxw9grdPqP7PAEA2uUdsyQ0ag0";
var supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);
async function saveChatMessage(userId, message, response, context = []) {
  try {
    const { data, error } = await supabase.from("fustonchats").insert({
      user_id: userId,
...
async function getChatHistory(userId, limit = 10) {
  try {
    const { data, error } = await supabase.from("fustonchats").select("*").eq("user_id", userId).order("created_at", { ascending: false }).limit(limit);
    if (error) {
...
async function clearChatHistory(userId) {
  try {
    const { error } = await supabase.from("fustonchats").delete().eq("user_id", userId);
    if (error) {
